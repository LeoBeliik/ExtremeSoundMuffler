...
public class ESMSlider extends AbstractWidget implements ISoundLists, IColorsGui {
    ...
    public void renderWidget(@NotNull GuiGraphics stack, int mouseX, int mouseY, float partialTicks) {
        ...
        //--------------- Render buttons BG ---------------//
        //stack.blit(RenderPipelines.GUI_TEXTURED, getIconsTextureRL(), btnMufflerX, this.getY(), isMuffling ? 143f : 109f, hoverMuffler ? 58f : 47f, sideBTNSize, sideBTNSize, 256, 256); //muffle button bg
        //stack.blit(RenderPipelines.GUI_TEXTURED, getIconsTextureRL(), btnPlayX, this.getY(), 98f, hoverPlay ? 58f : 47f, sideBTNSize, sideBTNSize, 256, 256); //play button bg
    ...
    private void setBtnToggleSound(String sound) {
        ...
        btnToggleSound = new ESMButton(x, getY() + 1, isMuffling ? 143 : 132, 47, 11, b -> {
            if (screen.btnBlocks.isSelected()) {
                Constants.loadBlockSounds(sound).forEach(this::setMufflingSounds);

                if (isMuffling && screen.btnBlocks.isSelected()) {
