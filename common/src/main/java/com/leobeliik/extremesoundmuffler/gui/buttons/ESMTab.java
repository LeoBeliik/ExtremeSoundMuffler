...
public class ESMTab extends Button implements IColorsGui {
    ...
    protected void extractContents(@NonNull GuiGraphicsExtractor GuiGraphicsExtractor, int mouseX, int mouseY, float v) {
        ...
        if (this.showing) {
            GuiGraphicsExtractor.blit(RenderPipelines.GUI_TEXTURED, getIconsTextureID(), x, y, 0f, 69f, 86, 16, 256, 256); //tab selected texure
            color = greenText; //text color
        } else {
            GuiGraphicsExtractor.blit(RenderPipelines.GUI_TEXTURED, getIconsTextureID(), x, y, 88f, 69f, 86, 16, 256, 256); //tab deselected texture
        }
        //Indicate if the button is being tagged
        GuiGraphicsExtractor.centeredText(Minecraft.getInstance().font, this.getMessage(), this.x + (this.width / 2), this.y + (this.height / 4), color);
    }
